...
module WebSocket =
    ...
    let send
        ...
        (cancellationToken: CancellationToken)
        =
        task { return! websocket.SendAsync(buffer, messageType, endOfMessage, cancellationToken) }
    ...
    let close
        ...
        (cancellationToken: CancellationToken)
        =
        task { return! websocket.CloseAsync(closeStatus, statusDescription, cancellationToken) }
            ...
            try
                while moreToRead do
                    ...
                    | result when
                        ...
                        mainResult <-
                            Closed(socket.CloseStatus.Value, socket.CloseStatusDescription)
                    | result ->
                        if
                            result.MessageType
                            <> messageType
                        then
                            failwithf
                                "Invalid message type received %A, expected %A"
                                result.MessageType
                                messageType
...
module ThreadSafeWebSocket =
    ...
    type SendMessages =
        | Send of
            ...
            IO.Stream *
            TaskCompletionSource<Result<unit, ExceptionDispatchInfo>>
        | Close of
            CancellationToken *
    ...
    type ReceiveMessage =
        CancellationToken * int * WebSocketMessageType * IO.Stream * TaskCompletionSource<Result<WebSocket.ReceiveStreamResult, ExceptionDispatchInfo>>
    ...
    let createFromWebSocket dataflowBlockOptions (webSocket: WebSocket) =
        ...
        let sendLoop = task {
            ...
            while webSocket
                ...
                | Close (cancellationToken, status, message, replyChannel) ->
                    ...
                    do!
                        wrap
                            (fun () -> WebSocket.close webSocket status message cancellationToken)
                            replyChannel
                | CloseOutput (cancellationToken, status, message, replyChannel) ->
                    ...
                    do!
                        wrap
                            (fun () ->
                                WebSocket.closeOutput webSocket status message cancellationToken
                            )
                            replyChannel
        }
        ...
        let receiveLoop = task {
            while webSocket
                  |> WebSocket.isWebsocketOpen do
                let! (cancellationToken, buffer, messageType, stream, replyChannel) =
                    receiveBuffer.ReceiveAsync()
        ...
        task {
            ...
            | Ok (WebSocket.ReceiveStreamResult.Stream s) ->
                return
                    ...
                    |> Ok
            | Ok (WebSocket.ReceiveStreamResult.Closed (status, reason)) ->
                return
